...
public class MNIST {
    ...
    public static final int NUM_TEST = 1_00;   // can be up to 10k
    ...
    public static class DataHolder implements Comparable<DataHolder> {
         ...
         * @return an integer that determines whether priority is <, > or =
         */
        @Override
        public int compareTo(DataHolder d) {
            return Float.compare(d.priority, priority);
        }
    }
     ...
     * @throws IllegalArgumentException if length of two arrays are different
     */
    public static float totalDist(float[] img1, float[] img2) throws IllegalArgumentException {
        ...
        for (int i = 0; i < img1.length; i++) {
            sum += (float) Math.pow(img1[i] - img2[i], 2);
        }
    ...
    public static DataHolder[] getClosestMatches(float[] image, int k) {
        PriorityQueue<DataHolder> pQueue = new PriorityQueue<>();
        for (int i = 0; i < TRAIN_IMAGES.length; i++) {
            float distance = totalDist(image, TRAIN_IMAGES[i]);
            DataHolder tempDataHolder = new DataHolder(TRAIN_LABELS[i], distance, TRAIN_IMAGES[i]);
            pQueue.offer(tempDataHolder);
            if (pQueue.size() > k) {
                pQueue.poll();
            }
        }
    ...
    public static int predict(DataHolder[] closestMatches) {
        int[] labelCountingArray = new int[10];
        for (DataHolder dh : closestMatches) {
            labelCountingArray[dh.label]++;
        }
        int closestLabel = 0;
        for (int i = 0; i < labelCountingArray.length; i++) {
            if (labelCountingArray[i] > labelCountingArray[closestLabel]) {
                closestLabel = i;
            }
        }
        return closestLabel;
